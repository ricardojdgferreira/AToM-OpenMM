...
parser.add_argument('--boxsize', required=False, type=str,
                    default=None,
                    help='Pass the box size to OpenMM (in case of membranes)')
...
if boxsize is not None:
    box_x = float(boxsize[1:].split(' ')[0])
    box_y = float(boxsize[1:].split(' ')[1])
    box_z = float(boxsize[1:].split(' ')[2])
...
if implsolv is None:
    ...
    if args['boxsize'] is not None:
        modeller.addSolvent(forcefield, boxSize=Vec3(box_x, box_y, box_z) * angstrom)
    else:
